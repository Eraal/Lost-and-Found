...
import { createLostItem, listItems, type CreateLostItemInput, type ItemDto, getSuggestionsForItem, type Suggestion, requestClaim } from '../../../lib/api'
...
export default function ReportLostPage() {
  ...
  async function loadRecent() {
    ...
    setLoadingList(true)
    try {
      const items = await listItems({ type: 'lost', reporterUserId: user.id, limit: 5 })
      setRecent(items)
    } catch {
      setRecent([])
    } finally {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSuccessId(null)
    try {
      const payload: CreateLostItemInput = {
      ...
      const created = await createLostItem(payload, user.id)
      setSuccessId(created.id)
      setForm({ title: '', description: '', location: '', occurredOn: '', photoFile: null })
      loadRecent()
      // Fetch smart suggestions for this lost item
      try {
            ...
            <div className="mb-6 flex items-center gap-3">
              <div className="relative inline-flex items-center justify-center size-9 rounded-full ring-2 ring-[color:var(--brand)]/20 bg-[color:var(--brand)]/5">
                ...
                <svg viewBox="0 0 24 24" className="size-5 text-[color:var(--brand)]" fill="currentColor" aria-hidden>
                  <path d="M6 7a3 3 0 0 1 3-3h6a3 3 0 0 1 3 3v10a2 2 0 0 1-2 2H8l-4 2V7a3 3 0 0 1 2-2 3 3 0 0 0 0 2Z" />
                </svg>
              </div>
              <div>
                <h1 className="text-xl sm:text-2xl font-semibold text-[color:var(--brand)]">Report a Lost Item</h1>
                <p className="text-sm text-[var(--ink-600)]">Provide details to help us identify and match your item.</p>
              </div>
            </div>

            {successId && (
              <div className="mb-4 rounded-lg bg-green-50 text-green-800 ring-1 ring-green-500/20 px-3 py-2 text-sm">
                Your report was submitted successfully. Reference ID: <span className="font-medium">{successId}</span>
              </div>
            )}
            ...
            <form className="space-y-4" onSubmit={onSubmit} noValidate>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Include unique identifiers, brand, color, etc."
                />
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="location" className="block text-sm font-medium text-[var(--ink)]">Last seen location</label>
                  <input
                    id="location"
                    name="location"
                    type="text"
                ...
                <div>
                  <label htmlFor="occurredOn" className="block text-sm font-medium text-[var(--ink)]">Date lost</label>
                  <input
                    id="occurredOn"
                    name="occurredOn"
                    type="date"
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <p className="text-sm text-[var(--ink-600)]">We’ll match your report with found items and notify you of any matches.</p>
              </div>
            </form>
            {/* Sidebar: recent reports */}
            <aside className="lg:border-l lg:pl-8 border-black/5 space-y-6">
              <h2 className="text-sm font-semibold text-[var(--ink)] mb-2">Your recent lost reports</h2>
              {loadingList ? (
                        ...
                        <li key={`${s.lostItemId}-${s.foundItemId}`} className="rounded-lg border border-black/10 bg-white/80 p-3">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="shrink-0 inline-flex items-center gap-2">
                              ...
                              <button onClick={async () => { if (!user) return; try { await requestClaim(s.foundItemId, user.id, 'Auto-match suggestion'); alert('Claim requested.'); } catch {/* ignore */} }} className="rounded-md px-2.5 py-1.5 text-xs font-semibold text-white bg-[color:var(--brand)] hover:bg-[color:var(--brand-strong)]">Verify Claim</button>
                            </div>
                          </div>
                        </li>
                      ))}
